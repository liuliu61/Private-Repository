...
test('运营费作为实际费用从利润中扣除', () => {
  ...
  assert.equal(result.grossProfit.toFixed(2), '-1100.00');
});

test('毛利正数和零分别标记为盈利与持平', () => {
  const calculator = new GrossProfitCalculator();
  assert.equal(calculator.calculate({ customerCashAmount: decimal('10000'), supplierCashAmount: decimal('9000') }).profitStatus, ProfitStatus.LOSS);
