...
export class ReceivingService {
  ...
  async confirmBankTransaction(id: string, context: AccessContext) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const receiveRecord = await tx.receiveRecord.create({ data: { receiveNo: this.generateReceiveNo(), organizationId: bank.organizationId, customerId: bank.matchedCustomerId, purchaseOrderId: bank.matchedPurchaseOrderId, accountId: bank.accountId, bankTransactionId: bank.id, amount: bank.amount, unBillingAmount: bank.amount, currency: bank.currency, receivedAt: bank.bookedAt || bank.occurredAt, status: bank.matchedCustomerId ? ReceiveRecordStatus.PENDING_CONFIRMATION : ReceiveRecordStatus.PENDING_MATCH, createdBy: context.sub } });
  ...
  async confirmReceiveRecord(id: string, dto: ReceivePostingDto = new ReceivePostingDto(), context?: AccessContext) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const publicTotal = publicDetails.reduce((sum, detail) => sum.add(detail.amount), new Prisma.Decimal(0));
      ...
      const updated = await tx.receiveRecord.update({ where: { id }, data: { status: ReceiveRecordStatus.CONFIRMED, transactionId: transaction.id, postedAmount: walletCreditAmount, refundedAmount: new Prisma.Decimal(0), serviceFeeAmount, walletCreditAmount, invoiceEligibleAmount: new Prisma.Decimal(0), unBillingAmount: publicTotal, billingAmount: new Prisma.Decimal(0), billedAmount: new Prisma.Decimal(0), confirmedBy: context.sub, confirmedAt: new Date(), remark: dto.remark?.trim() || row.remark || null }, include: { details: { include: { invoiceTask: true } } } });
  ...
  private postingView(row: any) { return { ...row, paymentAmount: moneyToString(row.paymentAmount), refundAmount: moneyToString(row.refundAmount ?? new Prisma.Decimal(0)), serviceFeeAmount: moneyToString(row.serviceFeeAmount ?? new Prisma.Decimal(0)), walletCreditAmount: moneyToString(row.walletCreditAmount ?? new Prisma.Decimal(0)), invoiceEligibleAmount: moneyToString(row.invoiceEligibleAmount ?? new Prisma.Decimal(0)) }; }
