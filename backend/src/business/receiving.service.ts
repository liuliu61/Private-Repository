...
import { BankTransactionDirection, BankTransactionStatus, Prisma, PurchasePaymentType, ReceiveRecordStatus, RefundStatus, TransactionBusinessType } from '@prisma/client';
...
export class ReceivingService {
  ...
  async getReceiveRecord(id: string, context: AccessContext) {
    this.assertPermission(context, 'FINANCE_RECEIVE_VIEW');
    const row = await this.prisma.receiveRecord.findUnique({ where: { id }, include: { customer: true, bankTransaction: true, purchaseOrder: { select: { id: true, orderNo: true, amount: true, status: true } }, account: true, transaction: true, creator: { select: { id: true, displayName: true } }, confirmer: { select: { id: true, displayName: true } }, postings: true, serviceFeeDetails: { include: { operator: { select: { id: true, displayName: true } } } } } });
  ...
  async confirmReceiveRecord(id: string, dto: ReceivePostingDto = new ReceivePostingDto(), context?: AccessContext) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.receiveRecord.update({ where: { id }, data: { status: ReceiveRecordStatus.CONFIRMED, transactionId: transaction.id, postedAmount: walletCreditAmount, refundedAmount: new Prisma.Decimal(0), serviceFeeAmount, walletCreditAmount, invoiceEligibleAmount: row.amount, confirmedBy: context.sub, confirmedAt: new Date(), remark: dto.remark?.trim() || row.remark || null } });
      await tx.bankTransaction.update({ where: { id: bank.id }, data: { status: BankTransactionStatus.RECEIPT_CONFIRMED } });
      await this.audit(tx, context, row.organizationId, id, 'RECEIVE_RECORD_POST', { status: row.status, postedAmount: moneyToString(row.postedAmount) }, { status: updated.status, postingNo: posting.postingNo, transactionNo: transaction.transactionNo, walletCreditAmount: moneyToString(walletCreditAmount), invoiceEligibleAmount: moneyToString(row.amount) });
  ...
  private receiveView(row: any) { return { ...row, amount: moneyToString(row.amount), postedAmount: moneyToString(row.postedAmount ?? new Prisma.Decimal(0)), refundedAmount: moneyToString(row.refundedAmount ?? new Prisma.Decimal(0)), serviceFeeAmount: moneyToString(row.serviceFeeAmount ?? new Prisma.Decimal(0)), walletCreditAmount: moneyToString(row.walletCreditAmount ?? new Prisma.Decimal(0)), invoiceEligibleAmount: moneyToString(row.invoiceEligibleAmount ?? new Prisma.Decimal(0)), serviceFeeDetails: row.serviceFeeDetails?.map((detail: any) => ({ ...detail, amount: moneyToString(detail.amount) })), postings: row.postings?.map((posting: any) => ({ ...posting, paymentAmount: moneyToString(posting.paymentAmount), refundAmount: moneyToString(posting.refundAmount), serviceFeeAmount: moneyToString(posting.serviceFeeAmount), walletCreditAmount: moneyToString(posting.walletCreditAmount), invoiceEligibleAmount: moneyToString(posting.invoiceEligibleAmount) })), transaction: row.transaction ? this.transactionView(row.transaction) : undefined }; }
