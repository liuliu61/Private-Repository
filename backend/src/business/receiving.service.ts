...
export class ReceivingService {
  ...
  async confirmReceiveRecord(id: string, dto: ReceivePostingDto = new ReceivePostingDto(), context?: AccessContext) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.receiveRecord.update({ where: { id }, data: { status: ReceiveRecordStatus.CONFIRMED, transactionId: transaction.id, postedAmount: walletCreditAmount, refundedAmount: new Prisma.Decimal(0), serviceFeeAmount, walletCreditAmount, invoiceEligibleAmount: row.amount, unBillingAmount: row.amount, billingAmount: new Prisma.Decimal(0), billedAmount: new Prisma.Decimal(0), confirmedBy: context.sub, confirmedAt: new Date(), remark: dto.remark?.trim() || row.remark || null } });
