...
export default function InvoiceProfilePanel({ token, customers, onError }: { token: string; customers: Customer[]; onError: (message: string) => void }) {
  ...
  async function refresh(id = customerId) { if (!id) return; setLoading(true); try { setProfiles(await request<Profile[]>(`/customers/${id}/invoice-profiles`, token)); } catch (error) { onError(error instanceof Error ? error.message : '开票信息查询失败'); } finally { setLoading(false); } }
  ...
  async function save(values: Record<string, unknown>) { if (!customerId) return; try { const path = editing ? `/customers/${customerId}/invoice-profiles/${editing.id}` : `/customers/${customerId}/invoice-profiles`; await request(path, token, { method: editing ? 'PATCH' : 'POST', body: JSON.stringify(values) }); setOpen(false); await refresh(); } catch (error) { onError(error instanceof Error ? error.message : '开票信息保存失败'); } }
  async function remove(row: Profile) { if (!customerId) return; try { await request(`/customers/${customerId}/invoice-profiles/${row.id}`, token, { method: 'DELETE' }); await refresh(); } catch (error) { onError(error instanceof Error ? error.message : '开票信息删除失败'); } }
