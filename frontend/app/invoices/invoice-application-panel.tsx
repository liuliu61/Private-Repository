...
type Application = { id: string; invoiceNo: string; amount: string; status: string; createdBy?: string; customer?: Customer; receiveSources?: Array<{ amount: string; receiveRecord?: { receiveNo: string } }>; applicationItems?: Array<{ amount: string; itemType?: string }>; createdAt: string; reviewedBy?: string; reviewedAt?: string; approvalRemark?: string; rejectReason?: string };
...
export default function InvoiceApplicationPanel({ token, customers, review = false, onError }: { token: string; customers: Customer[]; review?: boolean; onError: (message: string) => void }) {
  ...
  async function recognizeInvoice(file: any) {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/invoices/ocr`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.message || 'AI识别失败，请手工填写');
      const data = result.data || {};
      form.setFieldsValue({ invoiceNumber: data.invoiceNo || undefined, invoiceDate: data.invoiceDate || undefined, amount: data.totalAmount || data.amountExcludingTax || undefined, invoiceType: data.invoiceType || undefined, invoiceTitle: data.buyerName || undefined, taxNumber: data.buyerTaxNo || undefined, invoiceContent: data.items?.map((item: any) => item.content || item.name).filter(Boolean).join('、') || undefined });
      setOcrRecordId(result.rawResultId); setOcrMessage('AI识别完成，请核对识别结果');
  ...
  async function revokeAction(id: string) {
    try { await request(`/invoices/applications/${id}/revoke`, token, { method: 'POST' }); await refresh(filterForm.getFieldsValue()); }
    catch (error) { onError(error instanceof Error ? error.message : '发票申请撤回失败'); }
  }
