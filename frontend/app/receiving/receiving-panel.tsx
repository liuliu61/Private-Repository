  ...
  return <Card title="收款管理" loading={loading}>
    ...
    {tab === 'receive' && <>
      ...
      <Table rowKey="id" dataSource={receives} pagination={{ pageSize: 20, showSizeChanger: true, pageSizeOptions: [10, 20, 50, 100] }} columns={[
        ...
        { title: '操作', render: (_: unknown, row: ReceiveRecord) => <Space><Button type="link" onClick={() => void showReceiveDetail(row.id)}>详情</Button>{row.status === 'PENDING_CONFIRMATION' && <Button type="link" onClick={() => void action(`/receive-records/${row.id}/confirm`, '确认收款失败')}>确认收款</Button>}{row.transaction ? <span>{row.transaction.transactionNo}</span> : null}</Space> },
      ]} />
    </>}
