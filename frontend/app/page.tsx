...
export default function HomePage() {
  ...
  useEffect(() => { if (!token) return; Promise.all([request<Dashboard>('/dashboard', token), request<Account[]>('/accounts', token), request<{ items: Transaction[] }>('/transactions?page=1&pageSize=10', token), request<Customer[]>('/customers', token), request<Supplier[]>('/suppliers', token), request<{ items: ReceiveRecord[] }>('/receive-records?page=1&pageSize=50', token)]).then(([d, a, t, c, s, r]) => { setDashboard(d); setAccounts(a); setTransactions(t.items); setCustomers(c); setSuppliers(s); setReceiveRecords(r.items || []); }).catch((error) => message.error(error.message)).finally(() => setLoading(false)); }, [token, message]);
