...
export default function HomePage() {
  ...
  useEffect(() => { if (!token || (!customers.length && !suppliers.length)) return; const validCustomers = customers.filter((c: any) => c.agentId); Promise.allSettled([...validCustomers.map((item) => request<PromotionAccount[]>(`/customers/${item.id}/promotion-accounts`, token)), ...suppliers.map((item) => request<PromotionAccount[]>(`/suppliers/${item.id}/promotion-accounts`, token))]).then((results) => { const accounts = results.filter((r) => r.status === 'fulfilled').flatMap((r: any) => r.value || []); setPromotionAccounts(accounts); }); }, [token, customers, suppliers]);
