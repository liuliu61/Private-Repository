...
async function request<T>(path: string, token: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) throw new Error(Array.isArray(data.message) ? data.message.join('；') : data.message || '服务费对账请求失败');
  return data as T;
}
...
export default function ServiceFeePanel({ token, onError }: { token: string; onError: (error: string) => void }) {
  ...
  async function load(nextPage = page, nextFilters = filters, nextTab = tab) {
    ...
    try {
      ...
      const result = await request<{ items: ServiceFeeRow[]; totalCount: number; totalAmount: string; totalServiceFee: string }>(`/service-fee-reconciliation?${params.toString()}`, token);
  ...
  async function exportRows() {
    try {
      ...
      const response = await fetch(`${apiUrl}/service-fee-reconciliation/export?${params.toString()}`, { headers: { Authorization: `Bearer ${token}` } });
