...
export default function ProcurementOrderPanel({ token, customers, suppliers, onError }: { token: string; customers: Customer[]; suppliers: Supplier[]; onError: (message: string) => void }) {
  ...
  async function openPayment(order: Order, type: 'customer' | 'supplier') {
    ...
    paymentForm.setFieldsValue({ occurredAt: new Date().toISOString().slice(0, 16), idempotencyKey: crypto.randomUUID() });
    if (type === 'supplier') {
  ...
  async function openCredit(order: Order) {
    ...
    creditForm.setFieldsValue({ occurredAt: new Date().toISOString().slice(0, 16), idempotencyKey: crypto.randomUUID(), creditAmount: order.customerCreditRemaining || order.customerCreditAmount || '' });
  ...
  return <Card title="外采订单" extra={<Space><Button onClick={() => { listForm.resetFields(); setListFilters({}); void refresh({}); }}>重置</Button><Button type="primary" onClick={() => setModalOpen(true)}>新增订单</Button></Space>} loading={loading}>
    ...
    <Table rowKey="id" dataSource={orders} pagination={false} columns={[
      { title: '订单号', dataIndex: 'procurementNo' },
      { title: '客户', dataIndex: 'customerId', render: (value: string) => customers.find((item) => item.id === value)?.name || value },
      { title: '供应商', dataIndex: 'supplierId', render: (value: string) => suppliers.find((item) => item.id === value)?.name || value },
      { title: '平台', dataIndex: 'platform' },
      { title: '交易类型', dataIndex: 'transactionType', render: (value: string) => value === 'TRANSFER_OUT' ? '转出' : '转入' },
      { title: '业务类型', dataIndex: 'businessType' },
      { title: '基准金额', dataIndex: 'baseAmount' },
      { title: '客户返点', render: (_: unknown, row: Order) => row.customerRebateRate ? `${rebateTypeName[row.customerRebateType || ''] || row.customerRebateType} ${row.customerRebateRate}%` : '未配置' },
      { title: '成本政策', render: (_: unknown, row: Order) => row.supplierCostRate ? `${rebateTypeName[row.supplierRebateType || ''] || row.supplierRebateType} ${row.supplierCostRate}%` : '未配置' },
      { title: '状态', dataIndex: 'status', render: (value: string) => <Tag>{statusName[value] || value}</Tag> },
      { title: '操作', key: 'action', render: (_: unknown, row: Order) => <Space wrap><Button type="link" onClick={() => void showDetail(row.id)}>详情</Button>{row.status === 'DRAFT' && <><Button type="link" onClick={() => void transition(row.id, 'submit')}>提交确认</Button><Button type="link" danger onClick={() => void transition(row.id, 'cancel')}>取消</Button></>}{row.status === 'PENDING_CONFIRMATION' && <><Button type="link" onClick={() => void transition(row.id, 'confirm')}>确认</Button><Button type="link" danger onClick={() => void transition(row.id, 'cancel')}>取消</Button></>}{(row.status === 'CONFIRMED' || row.status === 'SETTLED') && <><Button type="link" onClick={() => void openPayment(row, 'customer')}>录入客户打款</Button><Button type="link" onClick={() => void openCredit(row)}>确认账户币到账</Button><Button type="link" onClick={() => void openPayment(row, 'supplier')}>支付一级代理</Button><Button type="link" onClick={() => void openRefund(row)}>申请退款</Button></>}{row.status === 'CONFIRMED' && <Button type="link" onClick={() => void transition(row.id, 'settle')}>标记已结算</Button>}</Space> },
    ]} />
    ...
    <Modal title={paymentType === 'customer' ? '录入客户打款' : '支付一级代理'} open={paymentOpen} onCancel={() => setPaymentOpen(false)} onOk={() => paymentForm.submit()} okText="确认入账" cancelText="取消">
      <Form form={paymentForm} layout="vertical" onFinish={submitPayment}>
        ...
        <Form.Item name="businessNo" label="业务单号" rules={[{ required: true, message: '请输入业务单号' }]}><Input /></Form.Item>
        <Form.Item name="idempotencyKey" label="幂等键" rules={[{ required: true, message: '请输入幂等键' }]}><Input /></Form.Item>
